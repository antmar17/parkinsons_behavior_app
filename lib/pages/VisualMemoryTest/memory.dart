                              ...
                              if (difficulty < 1) {
                                ...
                                if (widget.gridDimension < 6) {
                                  Navigator.pushReplacementNamed(
                                      context,
                                      "/memory" +
                                          widget.gridDimension.toString());
                                } else {
                                  Navigator.popUntil(
                                      context,
                                      (route) =>
                                          route.settings.name == '/difficulty');
                                }
                              }
                            }
                          },
    ...
    sub.onDone(() {
      if(_countDownCurrent == 0) {
        DataBaseService(uid: _authService
            .getCurrentUser()
            .uid).updateUserMemoryGame(score, widget.gridDimension - 1, false);
      }
      gameStarted = false;
    });
  }
...
class _TileState extends State<Tile> {
  ...
  bool checkForWin() {
    ...
    gameStarted = false;
    _timer!.cancel();
    DataBaseService(uid:widget.parent._authService.getCurrentUser().uid).updateUserMemoryGame(widget.parent.score,widget.parent.widget.gridDimension,true);
    return true;
  }
}
